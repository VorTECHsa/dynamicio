...
class S3CsvReaderWriter(BaseModel, Readable[pd.DataFrame], Writable[pd.DataFrame]):
    bucket: str
    path: Path
    ...
    def read(self) -> pd.DataFrame:
        ...
        if self.force_read_to_memory:
            df = pd.read_csv(self._s3_path, **self.read_kwargs)  # type: ignore
            if df is not None:
                return df
    ...
    def write(self, df: pd.DataFrame) -> None:
        """Write CSV to S3."""
        df.to_csv(self._s3_path, **self.write_kwargs)
